...
from src.features import make_main_pipeline
...
def train_main_model(tier_data: TierData, *, variant: str = DEFAULT_VARIANT):
    variant = normalize_variant(variant)
    model = make_main_pipeline(variant)
    model.fit(tier_data.train_texts, tier_data.train_labels)
    return model


def train_and_evaluate_main(
    tier_data: TierData,
    *,
    ...
    variant = normalize_variant(variant)
    model = train_main_model(tier_data, variant=variant)
...
def main() -> None:
    ...
    _, metrics = train_and_evaluate_main(
        tier_data,
